...
$("saveSettings").onclick=async()=>{try{collectSettings();settings=await fetch("/api/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(settings)}).then(readJson);$("settingsState").textContent="Saved. Live coach behavior updated. Spotter cache is warming.";renderSettings();await refreshAudioStatus()}catch(err){$("settingsState").textContent=err.message}};
