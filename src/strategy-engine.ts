...
export function analyzeStrategy(session: RecordedSession): StrategyReport {
  ...
  if (samples.length < 2) return { status: "insufficient-data", completedLaps: samples.length, fuelPerLapGallons: null, estimatedLapsRemaining: null,
    tireWearPerLapPercent: null, paceTrendSecondsPerLap: null, projectedStintLaps: null, virtualEnergyPerLapPercent: null, estimatedEnergyLapsRemaining: null,
    recommendation: "Complete two representative laps to establish fuel, energy, and tire trends." };
  ...
  const energyUsage = average(samples.map(sample => sample.virtualEnergyUsedPercent).filter((value): value is number => value !== null && value > .01));
  const energyRemaining = samples.at(-1)!.virtualEnergyRemainingPercent;
  const estimatedEnergyLapsRemaining = energyUsage > .01 && energyRemaining !== null ? energyRemaining / energyUsage : null;
  const status = (estimatedLapsRemaining !== null && estimatedLapsRemaining < 2) || (estimatedEnergyLapsRemaining !== null && estimatedEnergyLapsRemaining < 2) ? "pit-risk" : paceTrendSecondsPerLap > .8 || tireWearPerLapPercent > 2 ? "watch" : "stable";
  const recommendation = estimatedEnergyLapsRemaining !== null && estimatedEnergyLapsRemaining < 2 ? "Virtual energy range is below two laps. Reduce consumption and review the stint target."
    : status === "pit-risk" ? "Fuel range is below two laps. Plan to pit this lap."
  ...
  return { status, completedLaps: samples.length, fuelPerLapGallons: fuelPerLapLiters > .05 ? fuelPerLapLiters * LITERS_TO_GALLONS : null,
    estimatedLapsRemaining, tireWearPerLapPercent, paceTrendSecondsPerLap, projectedStintLaps,
    virtualEnergyPerLapPercent: energyUsage > .01 ? energyUsage : null, estimatedEnergyLapsRemaining, recommendation };
}

interface LapSample { fuelUsedLiters: number; fuelRemainingLiters: number; tireWearPercent: number; lapSeconds: number; virtualEnergyUsedPercent: number | null; virtualEnergyRemainingPercent: number | null; }
function lapSample(frames: TelemetryFrame[], lapSeconds: number): LapSample | null {
  ...
  const startEnergy = first.virtualEnergyPercent, endEnergy = last.virtualEnergyPercent;
  return { fuelUsedLiters: Math.max(0, first.fuelLiters - last.fuelLiters), fuelRemainingLiters: last.fuelLiters,
    ...
    virtualEnergyUsedPercent: startEnergy != null && endEnergy != null ? Math.max(0, startEnergy - endEnergy) : null,
    virtualEnergyRemainingPercent: endEnergy ?? null };
}
